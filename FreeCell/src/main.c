...
void setStackFromdeck(Element** deck,Element** listCol){
        card* c = peek(*deck);
        pop(deck);
        addcard(c,listCol);
}
...
int main(int argc, char** argv)
{
    ...
    SDL_Window* window = SDL_CreateWindow("FREECELL - by Naoufal", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
    ...
    Element* deck = Generatedeck(renderer);
    ...
    for(int i=0;i<7;i++){
        ...
        setStackFromdeck(&deck,&listcol1);
        setStackFromdeck(&deck,&listcol2);
        setStackFromdeck(&deck,&listcol3);
        setStackFromdeck(&deck,&listcol4);
        
        if(i != 6){
            setStackFromdeck(&deck,&listcol5);
            setStackFromdeck(&deck,&listcol6);
            setStackFromdeck(&deck,&listcol7);
            setStackFromdeck(&deck,&listcol8);
        }
    }
